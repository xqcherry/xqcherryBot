...
MIGRATION_DIR = Path(getattr(global_config, "db_migration_dir", "/app/migrations"))
DB_PATH = Path(getattr(global_config, "db_output_path", "/app/db/bot_data.db"))

@get_driver().on_startup
async def run_flyway_migration():
    logger.info(f"【Flyway】开始扫描迁移脚本目录: {MIGRATION_DIR}")
        ...
        for version, file_path in sql_files:
            if version > current_version:
                ...
                try:
                    ...
                    logger.error(f"【Flyway】脚本 {file_path.name} 执行失败！错误: {e}")
                    return
